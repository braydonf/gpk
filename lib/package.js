...
class Package {
  ...
  resolveRemote(options) {
    ...
    let git = null;
    const matched = src.match(/^(git\+(ssh\:\/\/|https\:\/\/|file\:\/\/)|git\:\/\/)(.*)$/);
  ...
  async uninstallGlobalModule(name) {
    const dst = path.join(this.env.globalLib, name);
  ...
  async uninstall(sources = [], options = {}) {
    if (options.global && sources.length > 0) {
      for (const source of sources) {
        const dst = path.join(this.env.globalLib, source);
  ...
  async copyPackage(src, dst) {
    ...
    for (const {pattern, always, inverse} of ignorePatterns) {
      ...
      if (!always && hasKeepPatterns && keep.has(this.env.normalizePath(file)))
        continue;
  ...
  async linkGlobal() {
    const libdir = this.env.globalLib;
  ...
  async linkBin(location, options = {}) {
    ...
    if (location && this.info.bin) {
      for (const [name, rel] of Object.entries(this.info.bin)) {
        ...
        if (options.global)
          libdir = path.join(this.env.globalLib, this.info.name);
