...
class Package {
  ...
  resolveRemote(name, prefix) {
    ...
    if (this.info.devDependencies) {
      if (this.info.devDependency[name]) {
        if (src != null)
          throw new Error(`Duplicate dependency ${name}.`);
        else
          src = this.info.devDependency[name];
      }
    }
  ...
  async install(prefix = null, options = {}) {
    ...
    const gyp = path.join(this.dir, './binding.gyp');
    let addon = false;
    try {
  ...
  async rebuild() {
    return new Promise((resolve, reject) => {
      execFile(NODE_GYP, ['rebuild'], {cwd: this.dir}, (err, stdout) => {
        if (err)
          reject(err);
        resolve(stdout);
      });
  ...
  async run(name) {
    ...
    return new Promise((resolve, reject) => {
      const child = spawn(cmd, args, {cwd: this.dir, stdio: 'inherit'});
