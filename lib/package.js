...
class Package {
  ...
  isBundleFile(base, file) {
    ...
    // Determine if any bundled dependencies are scoped.
    const scopes = [];
    for (const name of bundleDependencies) {
      const match = name.match(/^@([^\/]+)\/([^\/]+)$/);
      // References 'scope' of '@<scope>/<name>'.
      if (match)
        scopes.push(match[1]);
    }
    ...
    // Matches for either:
    // 1. '@<scope>/<name>' for '/node_modules/@<scope>/<name>/<path>'
    // 2. '<name>' for '/node_modules/<name>/<path>'
