...
class Package {
  ...
  async install(prefix = null, options = {}) {
    ...
    for (const [name, src] of Object.entries(dependencies)) {
      ...
      for (const url of git) {
        ...
        try {
          await access(moduleBase, fs.constants.R_OK & fs.constants.W_OK);
        } catch (err) {
          if (err.code !== 'ENOENT')
            throw err;

          await mkdir(moduleBase);
        }

        await this.copyRepoFiles(verified, modulePath);
  ...
  async getIgnorePatterns(dir) {
    ...
    for (const line of constants.ALWAYS_IGNORE)
      include.push(line);
  ...
  async getFilePatterns() {
    ...
    for (const line of this.info.files) {
      // Exclude anything that should always be ignored.
      if (constants.ALWAYS_IGNORE.includes(line))
        continue;
  ...
  async copyRepoFiles(src, dst) {
    const only = new Set();
    let onlyPatterns = ['**'];
    let hasFilePatterns = (this.info.files != null);

    if (hasFilePatterns)
      onlyPatterns = await this.getFilePatterns();
    ...
    for (const pattern of ignorePatterns) {
      ...
      for (const file of files) {
        // Do not override file patterns defined.
        if (hasFilePatterns && only.has(file))
          continue;
