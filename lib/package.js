...
const {ensureDir, ensureSymlink} = require('./common');
...
class Package {
  ...
  resolveRemote(prefix = null, name, src) {
    if (!prefix)
      prefix = this.dir;
    ...
    const host = this.info.remotes[remote];
  ...
  async installModule(prefix = null, name, src) {
    const {git, version} = this.resolveRemote(prefix, name, src);
    ...
    if (!hasVerified) {
      this.env.log(`Cloning '${name}' from '${git}' at '${tag}'.`);

      const unverified = `${verified}-unverified`;

      await cloneRepo(tag, git, unverified);
    ...
    await this.injectCompatMeta(modulePath, src, git, head);
  ...
  async install(prefix = null, options = {}) {
    ...
    for (const [name, src] of Object.entries(dependencies)) {
      const modulePath = await this.installModule(prefix, name, src);
      if (modulePath)
   ...
   * purposes and the data should not be used or read
   * for anything.
   * @param {String} dst
   * @param {String} src
   * @param {String} url
  ...
  async injectCompatMeta(dst, src, url, commit) {
    let data = null;
