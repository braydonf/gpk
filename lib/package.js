...
class Package {
  ...
  initGlobalDir() {
    if (process.env.PREFIX) {
      this.globalDir = process.env.PREFIX;
    } else {
      this.globalDir = path.dirname(process.execPath);

      if (process.platform !== 'win32') {
        this.globalDir = path.dirname(this.globalDir);

        if (process.env.DESTDIR)
          this.globalDir = path.join(process.env.DESTDIR, this.globalDir);
      }
    }
  ...
  async install(prefix = null, options = {}) {
    ...
    for (const [name, src] of Object.entries(dependencies)) {
      ...
      for (const url of git) {
        ...
        if (annotated)
          await verifyRepo(tag, null, dst, this.options.stdio);
        else
          await verifyRepo(null, commit, dst, this.options.stdio);
  ...
  async run(name) {
    ...
    this.log(`Running '${name}' at '${this.dir}' for ` +
             `'${this.info.name}@${this.info.version}'.`);
    this.log(`Command '${this.info.scripts[name]}'.`)
